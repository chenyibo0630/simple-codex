//! 二进制入口:加载 config,构造 `Provider` + `ModelClient` + `AppState`,启动 axum 服务。

use std::path::Path;
use std::sync::Arc;

use simple_codex_app_server::{AppState, serve};
use simple_codex_client::{ModelClient, Provider};
...
async fn main() -> Result<(), ApiError> {
    ...
    let provider = Provider::new(cfg.base_url, cfg.wire_api);
    let client = ModelClient::new(cfg.api_key, provider, cfg.model);
    let state = AppState {
